...
- **Fancybox is lazy-loaded only on gallery pages** — the `Layout.astro` script dynamically imports `@fancyapps/ui` when `.custom-md img, #post-cover img` exists; its core CSS is imported statically in frontmatter. On Swup `content:replace` (before hook), call `Fancybox.close()` and `Fancybox.unbind()` for the bound selectors so a stale lightbox never binds to old DOM.
